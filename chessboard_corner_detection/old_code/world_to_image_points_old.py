...
aruco_model_points = np.array([
							...
							#(	mW/2, 	mH/2, 	0.0		),             # NEW added center point bellow chessboard
...
tlx, tly = 11.70, 11.75 #11.75, 11.75 #12.25, 11.75	# inches
height = -0.75 #inches
chessboard_model_points = np.array([

							# 	x,  	y,   	z= -0.75
							#(	mW/2,	mH/2,	height		),             # NEW added center point bellow chessboard
...
# tmp
chessboard_model_center_point = np.array([
...
# tmp base measurments
bh, bw = 11.75, 10.75 # inches
tlx, tly = 10.5, 11.0	# inches
chesboard_lower_borner_model_corner_points  = np.array([
...
#
#
#		TODO: use linear interpolation bewteen points to increase number of points used when solving pnpransac
#
#
...
def draw_box(B, input_image, color=None):
...
def draw_box_diagnols(B, input_image, color=None):
	# B - 4x2 matrix following clockwise convention
	np_2_tupple_int = lambda t: tuple(int(e) for e in tuple(t))
...
if __name__ == '__main__':
	...
	for k,v in input_image_dict.items():
		...
		#
		#	TODO: extend detected aruco points with linear interpolation/regression
		#
		...
		left_vertical_line_point_indices = [0,3,12,15] # "m00, m03, m30, m33"
		right_vertical_line_point_indices = [5,6,9,10]
		top_horizontal_line_point_indices = [0,1,4,5]
		bottom_horizontal_line_point_indices = [15,14,11,10]
		positive_diagnol_line_point_indices = [15,13,7,5]
		negitive_diagnol_line_point_indices = [0,2,8,10]
	
		input_image = interpolate_additional_points(aruco_image_points, [0,3,12,15], input_image)
		input_image = interpolate_additional_points(aruco_image_points, [5,6,9,10], input_image)
		input_image = interpolate_additional_points(aruco_image_points, [0,1,4,5], input_image)
		input_image = interpolate_additional_points(aruco_image_points, [15,14,11,10], input_image)
		input_image = interpolate_additional_points(aruco_image_points, [15,13,7,5], input_image)
		input_image = interpolate_additional_points(aruco_image_points, [0,2,8,10], input_image)

		#
		# determine the center point on the aruco plane on the 2D image 
		# using the intersection of the two diagnol lines
		# 

		pd_line_image_points = aruco_image_points[positive_diagnol_line_point_indices]  # get image points that should lay on the line

		#	seperate x and y columns
		pd_x = pd_line_image_points[:,0]
		pd_y = pd_line_image_points[:,1]

		# perform linear regression and retreive polynomial coefficents for best fit line
		p1 = regression_line_poly_coeffs(pd_x,pd_y)

		nd_line_image_points = aruco_image_points[negitive_diagnol_line_point_indices]  # get image points that should lay on the line

		#	seperate x and y columns
		nd_x = nd_line_image_points[:,0]
		nd_y = nd_line_image_points[:,1]

		# perform linear regression and retreive polynomial coefficents for best fit line
		p2 = regression_line_poly_coeffs(nd_x,nd_y)

		# find point of intersection
		intersection_point = point_of_intersection(p1, p2)

		# tmp draw
		#center = tuple(intersection_point)
		#radius = 5
		#color = (0,255,0)
		#input_image = cv2.circle(input_image, center, radius, color, thickness=15, lineType=8, shift=0)



		# try adding this point to both the model points and the image points before solving pnpransac
		#reversed_aruco_image_points = aruco_image_points[::-1]
		#new_aruco_image_points = np.zeros((17,2))
		#new_aruco_image_points[0] = intersection_point
		#new_aruco_image_points[1:] = aruco_image_points
		...
		#input_image = draw_box_diagnols(check_aruco_image_points[[0,5,10,15]], input_image, color=(0,0,255))
		...
		# draw predicted chessboard center point
		#chessboard_center_image_point = world_to_image_point(chessboard_model_center_point, rotation_matrix, translation_vector, camera_matrix, dist_coeffs)
		#input_image = draw_point_matrix(chessboard_center_image_point, input_image, color=(0,255,0), thickness=25)

		# draw line between center on aruco plane and chessboard center point
		#np_2_tupple_int = lambda t: tuple(int(e) for e in tuple(t))
		#lineThickness = 2
		#input_image = cv2.line(input_image, tuple(intersection_point), np_2_tupple_int(chessboard_center_image_point[0]), (0,255,0), lineThickness)

		# draw chessboard frame
		input_image = draw_box(chessboard_corner_points, input_image)
		#input_image = draw_box_diagnols(chessboard_corner_points, input_image)

		# draw base of chessboard frame
		chessboard_base_model_image_points = world_to_image_point(chesboard_lower_borner_model_corner_points, rotation_matrix, translation_vector, camera_matrix, dist_coeffs)
		#input_image = draw_box(chessboard_base_model_image_points, input_image)
		#input_image = draw_box_diagnols(chessboard_base_model_image_points, input_image)
