...
output_video_path = "aruco_plane_polygon_videos/aruco_plane_polygon_4.mp4" # tmp in this file
...
if __name__ == '__main__':
	...
	c = 44.3713
	tol = 1.5
	chessboard_plane_z_offset = -0.9 #inches above aruco plane
	"""

	#chessboard_plane_z_offset = 0 #inches above aruco plane_from_3_points# tmp
	#chessboard_plane_z_offset = -36.0 #inches above aruco plane_from_3_points# tmp
	...
	# MOVIE_NUM - 12,13 - measuremnt
	l = 6.75
	a = 57.5
	b = 49.75
	c = 76.0
	tol = 1.5
	chessboard_plane_z_offset = 3.85 #inches bellow aruco plane



	aruco_model_points = aruco_measurements_to_model_points(l,  a, b, c, tol)
	...
	#
	#	TODO: choose from earliest images first
	#
	#	TODO: average between 3 user selections? if chessboard selection
	...
	assert(len(selected_image_point_matrix)) == 4
	...
	#	temporary solution: just subtract z coordinate diffrence from aruco model points
	chessboard_plane_aruco_model_points = aruco_measurements_to_model_points(l,  a, b, c, tol, z=chessboard_plane_z_offset)
	selected_points_camera = find_intersections_with_aruco_plane(selected_image_point_matrix, chessboard_plane_aruco_model_points, rotation_vector, translation_vector, camera_matrix, dist_coeffs)
	...
	for k,v in input_image_dict.items():
		...
		if aruco_image_points is not None:
			...
			#
			#	Test out saved aruco points
			#
			
			# project 3d world points of aruco corners to image points (this is the model from the current image pnpsolver)
			calculated_aruco_image_points = world_to_image_point(aruco_model_points, rotation_vector, translation_vector, camera_matrix, dist_coeffs)



			#
			# aruco model points on diffrent planes
			#

			# floor plane aruco model points 
			checking_floor_plane_aruco_model_points = aruco_measurements_to_model_points(l,  a, b, c, tol, z=24.0)
			checking_floor_plane_aruco_image_points = world_to_image_point(checking_floor_plane_aruco_model_points, rotation_vector, translation_vector, camera_matrix, dist_coeffs)

			# chessboard plane aruco model points
			checking_chess_plane_aruco_model_points = aruco_measurements_to_model_points(l,  a, b, c, tol, z=chessboard_plane_z_offset)
			checking_chess_plane_aruco_image_points = world_to_image_point(checking_chess_plane_aruco_model_points, rotation_vector, translation_vector, camera_matrix, dist_coeffs)
			...
			# draw detected aruco trianlge ingonoring pnpsolver	(actual)
			# indices 0,5,10
			input_image = draw_triangle(aruco_image_points[[0,5,10]], input_image, color=(255,0,0), lineThickness=10)

			# draw projected aruco trianlge using pnpsolver
			# indices 0,5,10
			input_image = draw_triangle(calculated_aruco_image_points[[0,5,10]], input_image, color=(0,255,0), lineThickness=7)

			# draw projected aruco trianlge using saved aruco world points
			input_image = draw_triangle(selected_points_projected_to_image[[0,5,10]], input_image, color=(0,0,255), lineThickness=5)	# hope

			#
			#	Try drawing measured aruco model triangle on the floor plane 
			#
			input_image = draw_triangle(checking_floor_plane_aruco_image_points[[0,5,10]], input_image, color=(0,0,255), lineThickness=5)	# hope
			
			#
			#	Try drawing measured aruco model points on the chessboard plane 
			#
			input_image = draw_triangle(checking_chess_plane_aruco_image_points[[0,5,10]], input_image, color=(255,0,0), lineThickness=3)	# hope
			"""
			...
			# draw aruco plane aruco boxes
			# indices 0,1,2,3 - etc
			input_image = draw_box(aruco_image_points[[0,1,2,3]], input_image)
			input_image = draw_box(aruco_image_points[[4,5,6,7]], input_image)
			input_image = draw_box(aruco_image_points[[8,9,10,11]], input_image)

			# draw floor plane aruco boxes
			input_image = draw_box(checking_floor_plane_aruco_image_points[[0,1,2,3]], input_image)
			input_image = draw_box(checking_floor_plane_aruco_image_points[[4,5,6,7]], input_image)
			input_image = draw_box(checking_floor_plane_aruco_image_points[[8,9,10,11]], input_image)

			#
			#	Draw connectors between floor plane and aruco plane aruco points
			#

			input_image = draw_box_legs(aruco_image_points[[0,1,2,3]], checking_floor_plane_aruco_image_points[[0,1,2,3]], input_image)
			input_image = draw_box_legs(aruco_image_points[[4,5,6,7]], checking_floor_plane_aruco_image_points[[4,5,6,7]], input_image)
			input_image = draw_box_legs(aruco_image_points[[8,9,10,11]], checking_floor_plane_aruco_image_points[[8,9,10,11]], input_image)
				
			#
			#	Draw selected chessboard point boxes
			#
			input_image = draw_box(selected_points_projected_to_image, input_image)
